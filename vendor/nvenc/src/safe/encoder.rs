...
impl Encoder {
    ...
    /// EOS flush with **no** output bitstream (matches legacy `nvenc` 0.1 behavior; prefer [`Self::flush_eos`]).
    pub fn end_encode(&self) -> Result<(), NVencError> {
        self.encoder.end_encode(std::ptr::null_mut())
    }

    /// Drain the encoder with an EOS picture using a valid bitstream buffer (required on many drivers).
    pub fn flush_eos(&self, output: &BitStream) -> Result<(), NVencError> {
        self.encoder.end_encode(output.buffer)
    }
...
impl EncoderInternal {
    ...
    pub fn end_encode(&self, output_bitstream: *mut c_void) -> Result<(), NVencError> {
        let mut params: MaybeUninit<NVencPicParams> = MaybeUninit::zeroed();
        unsafe { &mut *params.as_mut_ptr() }.version = NV_ENC_PIC_PARAMS_VER;
